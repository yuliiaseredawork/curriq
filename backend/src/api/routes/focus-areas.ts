...
function fireConsolidation(courseId: string, userId: string) {
  ...
  lambda
    ...
    .catch((e) =>
      console.error(
        "[focus-areas] failed to fire consolidation",
        String(e?.message ?? e),
      ),
    );
}
...
focusAreas.get("/:courseId/focus-areas", async (c) => {
  ...
  try {
    ...
    const active = (
      await Promise.all(
        canonical.filter((r) => r.state !== "MASTERED").map(toView),
      )
    ).sort((a, b) => b.priority - a.priority);
    const mastered = await Promise.all(
      canonical.filter((r) => r.state === "MASTERED").map(toView),
    );
    ...
    return c.json(
      { error: "FOCUS_AREAS_UNAVAILABLE", message: e.message },
      500,
    );
  }
});
...
focusAreas.post("/:courseId/focus-areas/:concept/answer", async (c) => {
  ...
  try {
    ...
    if (question.type === "mcq") {
      const correct =
        normalize(input.userAnswer) === normalize(question.answer);
      score = correct ? 100 : 0;
      feedback = {
        type: "mcq",
        correct,
    ...
    const now = new Date().toISOString();
    const completed = session.completedQuestions.filter(
    ...
    if (isComplete) {
      ...
      const { masteryScore, state } = applySessionResult(
        prevScore,
        session.score,
      );
