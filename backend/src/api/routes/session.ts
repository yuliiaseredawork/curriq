...
function fireConsolidation(courseId: string, userId: string) {
  ...
  lambda
    ...
    .catch((e) =>
      console.error(
        "[session] consolidation fire failed",
        String(e?.message ?? e),
      ),
    );
}
...
async function computeSession(
  ...
  for (const co of courses) {
    ...
    reviewedConcepts += records.filter(
    ...
    for (const r of chapterIdFilter ? [] : records) {
      ...
      const question =
        set.questions[(r.repetitions ?? 0) % set.questions.length];
      concepts.push({
        courseId: co.courseId,
        courseTitle: co.title,
  ...
  const retentionScore = totalConcepts
    ? Math.round((masteredConcepts / totalConcepts) * 100)
    : 0;
  const { learningProgress, breakdown } = blendProgress({
    avgConceptMastery: totalConcepts
      ? Math.round(masterySum / totalConcepts)
      : 0,
