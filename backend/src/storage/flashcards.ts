...
export type CardType =
  "definition" | "cloze" | "scenario" | "misconception" | "comparison";
...
export async function putCard(card: Flashcard) {
  const dueIndex =
    card.status === "ACTIVE" && card.nextReviewAt
      ? { dueBucket: "FLASHCARD", nextReviewAt: card.nextReviewAt }
      : {};
  await ddb.send(
    new PutCommand({
      TableName: TABLE(),
      Item: {
        pk: `USER#${card.userId}`,
        sk: cardSk(card.courseId, card.cardId),
        ...card,
        ...dueIndex,
      },
    }),
...
export async function listCardsForConcept(
  ...
  return (await listCards(userId, courseId)).filter(
    (c) => c.conceptSlug === conceptSlug,
  );
}
