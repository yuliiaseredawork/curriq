...
export default function ReviewsPage() {
    ...
    return (
      <Shell>
        <div className="rounded-xl border border-yellow-800 bg-yellow-950/30 p-6 text-yellow-200">
          Preparing your reviews… one moment.
        </div>
      </Shell>
    );
  if (loading)
      ...
      <Shell>
        <div className="rounded-xl border border-green-700 bg-green-950 p-6">
          ...
          <p className="text-gray-200 mt-1">
            {reviewed > 0
              ? `You reviewed ${reviewed} concept${reviewed === 1 ? "" : "s"} today.`
  ...
  if (!question)
    return (
      <Shell>
        <p className="text-gray-300">Nothing to review.</p>
      </Shell>
    );
        ...
        {feedback && (
          <div className="rounded-lg border border-gray-700 bg-gray-950 p-4 space-y-3">
            <div className="flex items-center justify-between">
              <span
                className={
                  feedback.score >= 70 ? "text-green-400" : "text-yellow-300"
                }
              >
              ...
              <span className="text-xs text-gray-400">
                Rated {feedback.quality} · next review in{" "}
                {feedback.intervalDays}d
              </span>
            </div>
            {feedback.feedback?.type === "rubric" ? (
              <>
                ...
                {feedback.feedback.missingConcepts?.length > 0 && (
                  <div>
                    ...
                    <ul className="list-disc pl-5 text-sm text-gray-300">
                      {feedback.feedback.missingConcepts.map(
                        (s: string, i: number) => (
                          <li key={i}>{s}</li>
                        ),
                      )}
              ...
              <ScannableText
                text={feedback.feedback?.explanation}
                keyTerms={keyTerms}
                className="text-gray-300"
              />
            )}
